...
peer chaincode invoke -o orderer0:7050 --ordererTLSHostnameOverride orderer0 --tls --cafile crypto-config/ordererOrganizations/example.com/orderers/orderer0.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C mychannel -n basic --peerAddresses peer1:7051 --tlsRootCertFiles crypto-config/peerOrganizations/org1.example.com/peers/peer1.org1.example.com/tls/ca.crt --peerAddresses peer2:7051 --tlsRootCertFiles crypto-config/peerOrganizations/org2.example.com/peers/peer2.org2.example.com/tls/ca.crt -c '{"function":"initLedger","Args":[]}'
